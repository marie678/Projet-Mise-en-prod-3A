...
ingredient_list: set = sorted(set(reformat(df['NER'])))
recipe_durations: set = sorted(set(df['CookTime_minutes']))
ratings: set = sorted(set(df['AggregatedRating']))
...
@st.cache_data
def search_recipes(original_df, filters:list, dict_columns: dict):
    """
...
with st.form("filter_form", clear_on_submit=True):
    ...
    ingredients = col1.multiselect("Choose one or more ingredient(s)", ingredient_list, default=st.session_state.selected_ingredients if st.session_state.selected_ingredients else None)
    ...
    if recipe_time: # default to 'Select recipe time'?
        st.write(recipe_time)
        st.write(recipe_durations.index(st.session_state.selected_duration))
        filters['recipe_durations'] = recipe_time

    rating = col3.selectbox("Choose a rating", ratings, index=ratings.index(st.session_state.selected_rating) if st.session_state.selected_rating else None)
    st.session_state.selected_rating = rating  # Update rating in session state
    if rating:
        filters['ratings'] = rating
    
    other = col4.selectbox("Choose aother", [1, 2, 3], index=None)
